    ...
    return (
      <div className="space-y-6 md:space-y-8 w-full pb-8 pr-4 relative" dir="rtl">
        <div className="space-y-6 text-right">
          ...
          <div className="bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700/80 rounded-2xl shadow-xs overflow-hidden">
            <div className="px-4 pt-4 sm:px-6 border-b border-slate-100 dark:border-gray-700 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
              ...
              <div className="pb-3 md:pb-0 self-end md:self-auto">
                <div className="relative flex items-center border border-slate-200 dark:border-gray-700 rounded-xl bg-slate-50 dark:bg-gray-900/50 px-3 h-10 w-40 focus-within:border-[#1b8b99] focus-within:bg-white dark:bg-gray-800 transition-all">
              ...
              <>
                            ...
                            <td className="px-6">
                              <div className="flex items-center gap-3">
                                <div className="relative shrink-0">
                                  {appt.patientImage ? (
                                    <img loading="lazy" decoding="async" width="40" height="40" src={appt.patientImage.startsWith("http") ? appt.patientImage : `${FILES_URL}/${appt.patientImage}`} alt={appt.patientName} className="w-10 h-10 rounded-full object-cover" />
                                  ) : (
                                    <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 dark:text-gray-500 font-bold flex items-center justify-center text-xs border border-slate-200">{getInitials(appt.patientName)}</div>
                                  )}
                                </div>
                                <span className="font-bold text-gray-800 dark:text-gray-200 text-base">{appt.patientName}</span>
                              </div>
                            </td>
                            <td className="px-6">
                              <div className="flex flex-col text-right">
                                <span className="text-gray-900 dark:text-white font-bold text-sm">{dt.time}</span>
                                <span className="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5">{dt.date}</span>
                              </div>
                            </td>
                            <td className="px-6 text-gray-600 dark:text-gray-400 dark:text-gray-500 font-medium">{appt.visitType || '-'}</td>
                            <td className="px-6">
                ...
                <div className="block md:hidden divide-y divide-slate-100">
                  {appointments.map((appt) => {
                    ...
                    const dt = formatDateTime(appt.startTime);
                    return (
                      <div key={appt.id} className="p-4 space-y-4 hover:bg-slate-50 dark:bg-gray-900/20 transition-all text-right">
                          ...
                          <div className="flex items-center gap-3">
                            <div className="relative shrink-0">
                              {appt.patientImage ? (
                                ...
                                <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 dark:text-gray-500 font-bold flex items-center justify-center text-xs">{getInitials(appt.patientName)}</div>
                              )}
                            </div>
                            <span className="font-bold text-gray-800 dark:text-gray-200 text-sm">{appt.patientName}</span>
                          </div>
              ...
              <div className="flex items-center gap-1.5" dir="ltr">
                ...
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i + 1} onClick={() => setCurrentPage(i + 1)}
                    className={`w-8 h-8 rounded-lg text-xs font-bold flex items-center justify-center transition-all cursor-pointer ${currentPage === i + 1 ? "bg-[#1b8b99] text-white" : "border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 dark:text-gray-500 hover:bg-slate-50"}`}>
                    {i + 1}
                  </button>
                ))}
                <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages))}
                  className={`w-8 h-8 rounded-lg border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-center text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:bg-slate-50 dark:bg-gray-900 transition-colors ${currentPage === totalPages ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}>
