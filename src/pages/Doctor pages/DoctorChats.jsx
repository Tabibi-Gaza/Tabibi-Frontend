...
export default function DoctorChats() {
  ...
  return (
    <>
      ...
      `}</style>
      <div className="w-full pr-4 bg-slate-50 dark:bg-gray-900/30 flex justify-center items-start" dir="rtl">
        <div className="w-full bg-white dark:bg-gray-800 border border-[#e9eff6] dark:border-gray-700 rounded-2xl shadow-sm grid grid-cols-1 md:grid-cols-[1.2fr_2.5fr] min-h-[600px] h-[calc(100vh-180px)] md:h-[700px] overflow-hidden">
          ...
          <div className={`flex flex-col min-h-0 bg-white dark:bg-gray-800 custom-scrollbar h-full ${!showChatList ? 'flex' : 'hidden md:flex'}`}>
            {activeConversation ? (
              <>
                ...
                <div ref={messagesContainerRef} className="flex-1 min-h-0 p-4 md:p-6 overflow-y-auto custom-scrollbar bg-slate-50 dark:bg-gray-900/40 flex flex-col gap-4">
                  {messagesLoading ? (
                    <div className="flex items-center justify-center py-12"><Loader2 className="w-6 h-6 text-[#1b8b99] animate-spin" /></div>
                  ) : messages.length === 0 ? (
                    ...
                    messages.map((msg) => (
                      <div key={msg.id} className={`flex w-full ${msg.isMine ? 'justify-start' : 'justify-end'}`}>
                        <div className={`max-w-[85%] md:max-w-[70%] py-2.5 px-4 md:px-5 rounded-2xl text-sm font-semibold shadow-2xs ${msg.isMine ? 'bg-[#1b8b99] text-white rounded-br-none text-right' : 'bg-white dark:bg-gray-800 border border-[#e9eff6] dark:border-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-none text-right'}`}>
                ...
                <form onSubmit={handleSendMessage} className="p-3 md:p-4 border-t sticky border-[#e9eff6] dark:border-gray-700 bg-white dark:bg-gray-800 shrink-0">
                  <div className="flex items-center border border-[#e9eff6] dark:border-gray-700 rounded-xl bg-slate-50 dark:bg-gray-900 px-3 h-12 md:h-14 gap-2 md:gap-3 focus-within:border-[#1b8b99] focus-within:bg-white dark:bg-gray-800 transition-all">
