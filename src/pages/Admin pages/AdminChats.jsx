...
export default function AdminChats() {
  ...
  const renderMessageContent = (msg) => {
    ...
    if (msg.filePath && msg.fileType === "document") {
      return (
        <a
          href={resolveImageUrl(msg.filePath)}
          target="_blank"
  ...
  return (
    <div
      className="w-full flex flex-col items-start p-0 relative text-right"
      dir="rtl"
    >
      <div className="w-full flex-1 p-2 md:p-6 grid grid-cols-1 md:grid-cols-[1fr_2.5fr] gap-4 md:gap-6">
              ...
              filteredConversations.map((conv) => {
                ...
                return (
                  <div
                    ...
                    className={`group p-4 flex items-start justify-between border-b border-gray-50 cursor-pointer transition-all duration-200 ${isSelected ? "bg-[#e2f4f7] dark:bg-gray-800 md:border-r-4 md:border-r-[#138C9F]" : "hover:bg-slate-50 dark:bg-gray-900/50"}`}
                  >
                    <div className="flex items-center gap-3 min-w-0 text-right">
                      <div className="relative shrink-0">
                        {conv.participantImageUrl ? (
                                <img loading="lazy" decoding="async" width="44" height="44" className="w-11 h-11 rounded-full object-cover border border-gray-100" src={resolveImageUrl(conv.participantImageUrl)} alt={conv.participantName} />
                        ) : (
                    ...
                    <div className="flex flex-col items-end gap-1 shrink-0">
                      <span className="text-[10px] text-gray-400 dark:text-gray-500 font-semibold">{formatTime(conv.lastMessageTime)}</span>
                      {conv.unreadCount > 0 && (
        ...
        <div className={`bg-white dark:bg-gray-800 border border-[#e9eff6] dark:border-gray-700 rounded-2xl shadow-sm flex flex-col overflow-hidden h-full ${!showChatOnMobile ? "hidden md:flex" : "flex"}`}>
          {activeConversation ? (
            <>
              <div className="p-4 border-b border-[#e9eff6] dark:border-gray-700 flex items-center justify-between bg-white dark:bg-gray-800 shrink-0">
                <button onClick={() => setShowChatOnMobile(false)} className="block md:hidden p-2 text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:bg-slate-100 dark:bg-gray-800 rounded-full transition-colors">
                  <ArrowRight className="w-5 h-5" />
                </button>
                <div className="flex items-center gap-3 text-right ml-auto">
                  <div className="relative">
                    {activeConversation.participantImageUrl ? (
                            <img loading="lazy" decoding="async" width="44" height="44" className="w-10 h-10 md:w-11 md:h-11 rounded-full object-cover border border-gray-100" src={resolveImageUrl(activeConversation.participantImageUrl)} alt={activeConversation.participantName} />
                    ) : (
              ...
              {/* Messages */}
              <div className="flex-1 overflow-y-auto bg-slate-50 dark:bg-gray-900/30 flex flex-col gap-4 pb-24">
                {messagesLoading ? (
                  ...
                  messages.map((msg) => (
                    <div key={msg.id} className={`flex ${msg.isMine ? "justify-start" : "justify-end"}`}>
                      <div className={`py-2.5 px-4 md:py-3 md:px-5 rounded-2xl text-sm md:text-[14px] font-medium ${msg.isMine ? "bg-[#0052cc] text-white rounded-br-none" : "bg-[#e5eeff] text-gray-800 dark:text-gray-200 rounded-bl-none"}`}>
              ...
              {selectedFile && (
                <div className="px-4 py-2 border-t border-[#e9eff6] dark:border-gray-700 bg-slate-50 dark:bg-gray-900 flex items-center gap-3">
                  {filePreview ? (
                            <img loading="lazy" decoding="async" width="48" height="48" src={filePreview} alt="معاينة" className="w-12 h-12 rounded-lg object-cover" />
                  ) : (
              ...
              {/* Input */}
<form onSubmit={handleSendMessage} className="p-3 md:p-4 border-t pb-20 border-[#e9eff6] dark:border-gray-700 bg-white dark:bg-gray-800 shrink-0 max-w-full">
                  <div className="flex items-center border border-[#e9eff6] dark:border-gray-700 rounded-xl bg-slate-50 dark:bg-gray-900 px-3 h-12 md:h-14 gap-2 md:gap-3 focus-within:border-[#138C9F] transition-colors">
                  ...
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="text-gray-400 dark:text-gray-500 hover:text-[#138C9F] transition-colors cursor-pointer shrink-0">
            ...
            <div className="flex-1 overflow-y-auto">
              {doctorsLoading ? (
                ...
                doctors.map((doc) => {
                  ...
                  return (
                    <div key={doc.id} onClick={() => handleStartChatWithDoctor(doc)} className="p-4 flex items-center gap-3 border-b border-gray-50 cursor-pointer hover:bg-slate-50 dark:bg-gray-900/50 transition-all duration-200">
                      <div className="relative shrink-0">
                        {doc.profileImageUrl ? (
                            <img loading="lazy" decoding="async" width="44" height="44" className="w-11 h-11 rounded-full object-cover border border-gray-100" src={resolveImageUrl(doc.profileImageUrl)} alt={doc.fullName} />
                        ) : (
