...
const Favorites = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pt-28 pb-16 px-4" dir="rtl">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => navigate(-1)} className="w-9 h-9 rounded-full border border-gray-200 dark:border-gray-700 flex items-center justify-center text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:text-gray-400 dark:text-gray-500 hover:border-gray-300 transition-all cursor-pointer">
        ...
        {/* Tabs */}
        <div className="flex gap-2 mb-8 bg-gray-100 dark:bg-gray-800 p-1 rounded-xl">
          {TABS.map(tab => {
              ...
              <button
                ...
                className={`flex-1 py-3 px-4 rounded-xl text-sm font-bold transition-all cursor-pointer ${
                  activeTab === tab.key
                    ...
                    : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:text-gray-300 dark:text-gray-500'
                }`}
              >
                {tab.label} ({count})
              </button>
        ...
        {currentItems.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 dark:text-gray-500 text-sm mb-2">{currentTab.emptyMsg}</p>
            <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {currentItems.map(item => (
              <div
                key={item.id}
                className="bg-white dark:bg-gray-800 border border-gray-150 rounded-2xl shadow-xs hover:shadow-md transition-all duration-300 flex flex-col relative"
              >
                {/* شارة التوفر */}
                {item.available !== false && (
                  <div className="absolute top-3 right-3 z-20 flex items-center gap-1.5 text-[10px] font-bold text-white bg-emerald-600 px-2.5 py-1 rounded-full shadow-xs">
                    <span className="w-1.5 h-1.5 rounded-full bg-white dark:bg-gray-800 inline-block animate-pulse"></span>
                ...
                {/* الصورة */}
                <div className="bg-gray-50 dark:bg-gray-900/50 h-52 w-full flex items-center justify-center relative border-b border-gray-100">
                  {item.image ? (
                ...
                <div className="p-4 flex-1 flex flex-col justify-between text-right bg-white dark:bg-gray-800 relative">
                  <div className="space-y-1">
                    <div className="flex items-center justify-between">
                      <h3 className="text-gray-900 dark:text-white font-black text-sm">{item.name}</h3>
                      {item.rating && (
                        <div className="flex items-center gap-0.5 text-[10px] text-gray-500 dark:text-gray-400 dark:text-gray-500 font-bold">
                    ...
                    <div className="flex items-center gap-1 text-[11px] text-gray-500 dark:text-gray-400 dark:text-gray-500 font-bold pt-1">
                      <FontAwesomeIcon icon={faMapMarkerAlt} className="text-gray-400 dark:text-gray-500 text-xs" />
                      <span>{item.address || item.location || ""}</span>
                    </div>
                    {item.phone && (
                      <div className="flex items-center gap-1 text-[11px] text-gray-500 dark:text-gray-400 dark:text-gray-500 font-bold">
                        <FontAwesomeIcon icon={faClock} className="text-gray-400 dark:text-gray-500 text-xs" />
                  ...
                  <div className="flex items-center justify-between pt-3 mt-2 border-t border-gray-50">
                    {item.fees && (
                      <div className="text-right">
                        <p className="text-[10px] text-gray-400 dark:text-gray-500 font-bold leading-none">كشفية</p>
