      ...
      >
        <main className="max-w-233.75 w-full mx-auto px-4 sm:px-6 py-8 md:py-12 bg-white pb-10">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 mb-10 text-center sm:text-right sm:pr-8">
            {/* تم تعديل هذا الديف لإدراج صورة علامة الاستفهام الخضراء المطابقة لـ image_c488bd.png بدقة عالية */}
            <div className="grid place-items-center w-15 h-15 border border-dashed stroke-4 border-[#138C9F] rounded-full bg-linear-to-b from-[rgba(240,253,250,0.5)] to-transparent shadow-md">
              <FontAwesomeIcon
                icon="question" // استدعاء مباشر كـ String بدون import في هذا الملف!
                className="w-20 h-20 text-[#138C9F] stroke-[3]"
              />
            </div>
          ...
          <section className="w-full max-w-196.5 mx-auto mb-12">
            ...
            <div className="flex items-center gap-3 w-full h-16 px-5 mb-4 border border-[#f3f4f6] rounded-xl bg-white shadow-xs focus-within:border-[#c7e6f3] transition-all duration-200">
              ...
              />
              <svg
                className="w-5 h-5 text-[#94a3b8] flex-none"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
          ...
          <section className="w-full max-w-211.5 mx-auto p-6 sm:p-10 rounded-2xl bg-linear-to-r from-[#fafdfd] via-[#f3f8f8] to-[#eaf2f2] border border-[#edf3f7] shadow-xs flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden">
            <div className="flex-1 text-center md:text-right order-2 md:order-1">
              ...
              <p className="m-0 text-[#64748b] text-sm sm:text-base font-semibold leading-7 mb-6 max-w-139.5">
                فريق الخبراء لدينا جاهز لمساعدتك في أي استفسار تقني أو طبي. لا
                تتردد في مراسلتنا وسنقوم بالرد عليك في أسرع وقت ممكن.
              </p>
              <button
                type="button"
                className="w-full sm:w-88.25 h-13 rounded-xl bg-[#138C9F] text-white text-base font-extrabold shadow-md hover:bg-[#0c5f6c] active:scale-[0.98] transition-all duration-200 cursor-pointer"
              >
                تواصل معنا
              </button>
            </div>
