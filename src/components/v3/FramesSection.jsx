    ...
    <section className="relative h-screen w-full overflow-hidden bg-[#0a2540]" dir="rtl">
      ...
      <div className="relative h-full flex flex-col items-center justify-center px-4 sm:px-6" style={{ zIndex: 2 }}>
        <div className="w-full max-w-4xl text-center mb-8 md:mb-10">
          ...
          <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-white leading-tight drop-shadow-[0_4px_16px_rgba(0,0,0,0.5)]">
            رحلة صحتك تبدأ
          </h2>
          <p className="text-base sm:text-xl md:text-2xl text-[#b3e5e8] font-bold mt-3 drop-shadow-[0_2px_8px_rgba(0,0,0,0.4)]">
        ...
        <div className="w-full max-w-3xl bg-white/95 backdrop-blur-sm shadow-[0_20px_60px_rgba(0,0,0,0.25)] rounded-2xl p-2.5 sm:p-3 border border-white/20">
          <form onSubmit={handleSearch} className="flex flex-col md:flex-row items-center gap-2">
            <div className="w-full flex-1 relative">
              ...
              >
                <div className="flex items-center gap-3 select-none overflow-hidden">
                  ...
                  <span className={`font-extrabold text-right tracking-wide text-xs sm:text-sm md:text-base truncate ${
                ...
                <div className="absolute top-[115%] left-0 right-0 bg-white border border-slate-100 rounded-2xl p-2 shadow-[0_20px_50px_rgba(0,0,0,0.15)] z-50 grid grid-cols-1 sm:grid-cols-2 gap-1 max-h-72 overflow-y-auto">
                  ...
                  {options.map((opt) => (
                    <div key={opt.id} onClick={() => { setSpeciality(opt.id); setIsOpen(false); }}
                      className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl cursor-pointer font-bold text-xs md:text-sm transition-all select-none ${
            ...
            <button type="submit" disabled={isLoading}
              className="w-full md:w-auto bg-[#138C9F] text-white font-black text-sm md:text-base px-10 py-3.5 rounded-xl md:rounded-full hover:bg-[#0f7282] hover:shadow-lg hover:shadow-[#138C9F]/20 transition-all duration-200 active:scale-[0.98] whitespace-nowrap shrink-0 disabled:opacity-50 cursor-pointer">
              بحث سريع
            </button>
          </form>
        </div>
