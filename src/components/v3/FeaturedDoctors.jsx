...
function FeaturedDoctors() {
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-20 md:py-28 text-slate-900 dark:text-gray-200" dir="rtl">
      ...
      <h2 className="text-2xl sm:text-4xl font-black text-slate-900 dark:text-gray-200 mt-2 tracking-tight">
        الأطباء الأعلى تقييماً
      </h2>
      <p className="w-11/12 sm:w-1/2 text-center text-xs sm:text-sm text-slate-500 dark:text-gray-400 leading-relaxed max-w-xl px-4 sm:px-0 font-bold">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pt-12 px-4 max-w-7xl mx-auto">
        {loading ? (
          [...Array(4)].map((_, i) => (
            <div key={i} className="bg-white dark:bg-gray-800 border border-slate-100 dark:border-gray-700 rounded-3xl overflow-hidden shadow-[0_4px_25px_rgba(0,0,0,0.01)] flex flex-col h-full">
          ...
          <p className="col-span-full text-center text-slate-400 dark:text-gray-500 font-bold text-sm py-10">لا توجد بيانات متاحة حالياً</p>
        ) : (
          doctors.map((item, index) => (
            ...
            >
              <div className="bg-linear-to-b from-slate-50 to-white dark:from-gray-700 dark:to-gray-800 overflow-hidden w-28 sm:w-36 md:w-full aspect-square md:h-56 shrink-0 flex items-center justify-center relative border-l border-slate-100 dark:border-gray-700 md:border-l-0 md:border-b">
                <img
                  loading="lazy"
                  decoding="async"
                  width="224"
              ...
              <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between gap-3 text-right bg-white dark:bg-gray-800 relative z-10 min-w-0">
                ...
                <div className={`flex items-center gap-1.5 text-[10px] md:text-xs font-black mb-2 w-fit px-2.5 py-1 rounded-lg border select-none ${
                  ...
                  <h3 className="text-slate-900 dark:text-gray-200 font-black text-sm md:text-base flex items-center gap-1.5 group-hover:text-[#138C9F] transition-colors truncate">
                    <span className="truncate">{item.fullName}</span>
                  </h3>

                  <div className="flex items-center gap-2 mt-1.5 select-none">
                    {renderStars(item.averageRating)}
                    <span className="text-xs font-black text-slate-800 dark:text-gray-300 mt-0.5">{Number(item.averageRating || 0).toFixed(1)}</span>
                  ...
                  <p className="text-slate-400 dark:text-gray-500 text-[11px] md:text-xs mt-1.5 font-bold truncate">{item.yearsOfExperience ? `${item.yearsOfExperience} سنوات خبرة` : ""} من الخبرة والكفاءة</p>
                </div>
