...
const DoctorNavbar = ({ onMenuToggle, isMenuOpen }) => {
  ...
  return (
    <div
      className="h-16 md:h-21.5 bg-white dark:bg-gray-800 border-b border-[#C3C6D6] dark:border-gray-700 flex items-center justify-between px-4 sm:px-6 sticky top-0 z-45 w-full transition-colors"
    >
      ...
      <div className="flex items-center gap-3 sm:gap-6">
        <div className="relative">
          <div
            ...
            className="relative cursor-pointer transition-all hover:text-[#138C9F] p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700"
          >
          ...
          {showDropdown && (
            <>
              ...
              />
              <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-[360px] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-2xl rounded-2xl z-50 text-right overflow-hidden md:absolute md:top-12 md:left-0 md:translate-x-0 md:translate-y-0 md:w-[360px]">
                {/* Header */}
                <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-white dark:bg-gray-800">
                  <h3
                    ...
                    className="text-sm font-black text-[#0B1C30] dark:text-gray-200 cursor-pointer hover:text-[#138C9F]"
                  >
                ...
                {recentNotifications.length === 0 ? (
                    ...
                    <div className="w-12 h-12 bg-slate-100 dark:bg-gray-700 rounded-full flex items-center justify-center mx-auto mb-3">
                      <FiBell className="w-5 h-5 text-gray-300 dark:text-gray-500" />
                    </div>
                    <p className="text-sm font-bold text-gray-400 dark:text-gray-500">لا توجد إشعارات</p>
                  </div>
                ) : (() => {
                  const groups = {};
                  ...
                  return groupedArr.slice(0, 5).map((group) => {
                    ...
                    return (
                      <div
                        key={group.type}
                        onClick={handleGoToNotificationsPage}
                        className={`p-3.5 hover:bg-slate-50/70 dark:hover:bg-gray-700 cursor-pointer block border-b border-gray-50 dark:border-gray-700 last:border-0 transition-all ${
                          !group.isRead ? 'bg-blue-50/30 dark:bg-blue-900/20' : 'bg-white dark:bg-gray-800'
                        }`}
                      >
                        <div className="flex items-start gap-3">
                          {/* Icon */}
                          <div className={`w-9 h-9 rounded-xl ${config.bg} dark:bg-gray-700 flex items-center justify-center shrink-0`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                              ...
                              <p className="text-[13px] font-bold text-gray-800 dark:text-gray-200 leading-relaxed truncate">
                                {group.title}
                                {group.count > 1 && (
                            ...
                            <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5 truncate">{group.message}</p>
                            <span className="text-[10px] text-gray-400 dark:text-gray-500 font-semibold mt-1 block">
        ...
        <div
          className="flex items-center gap-2 md:gap-3 cursor-pointer p-1.5 px-2.5 rounded-xl border border-transparent hover:bg-gray-50/60 dark:hover:bg-gray-700/60 hover:border-[#138C9F] transition-all duration-300 select-none"
          onClick={handleProfileClick}
        >
          <div
            className="w-9 h-9 md:w-11 md:h-11 rounded-full border border-[#C3C6D6] dark:border-gray-600 bg-slate-100 dark:bg-gray-700 overflow-hidden relative group transition-all duration-300 hover:border-[#138C9F] shadow-sm shrink-0 flex items-center justify-center"
