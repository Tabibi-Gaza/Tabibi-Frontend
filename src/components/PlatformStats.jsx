...
const PlatformStats = () => {
   ...
   const statsData = [
     {
       id: 1,
       value: `+${doctors?.length || 15}`,
       label: "طبيب استشاري نَشِط",
       icon: faUserMd, // أيقونة الطبيب
     },
     {
       id: 2,
       value: "98٪",
       label: "نسبة رضا المرضى",
       icon: faStar, // أيقونة النجمة للتقييم
     },
     {
       id: 3,
       value: "15 دقيقة",
       label: "متوسط زمن الاستجابة",
       icon: faClock, // أيقونة الساعة للوقت
     },
     {
       id: 4,
       value: "+10000",
       label: "استشارة طبية ناجحة",
       icon: faCheckCircle, // أيقونة علامة الصح للنجاح والإنجاز
     },
   ];
            ...
            {statsData.map((stat) => (
              ...
              >
                ...
                <div className="w-12 h-12 rounded-xl flex items-center justify-center text-xl shrink-0 bg-slate-50 border border-slate-100/80 group-hover:bg-[#138C9F] group-hover:border-[#138C9F]/20 transition-all duration-300 transform group-hover:scale-105">
                  ...
                  <FontAwesomeIcon
                    icon={stat.icon}
                    className="text-[#138C9F] group-hover:text-white transition-colors duration-300"
                  />
                </div>
              </div>
            ))}
          ...
          <div className="bg-white border border-slate-100/80 rounded-3xl p-8 lg:p-12 shadow-[0_10px_40px_rgba(0,0,0,0.015)] flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-10 relative overflow-hidden">
            <div className="lg:w-1/3 flex flex-col items-start justify-center text-right z-10">
              ...
              <h3 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight mb-4 leading-tight">
                رعايتك الصحية في <br className="hidden md:block" /> 3 خطوات
                بسيطة
              </h3>
              <p className="text-sm text-slate-600 leading-relaxed font-bold mb-6 max-w-sm">
                قُمنا بهندسة المنصة وإلغاء التعقيد الورقي لتتمكن من الوصول إلى
                طبيبك المفضل بأقل جهد ممكن وبأعلى كفاءة رقمية.
              </p>
              ...
              <button
                ...
                className="text-sm font-black text-[#138C9F] hover:text-[#2d7792] flex items-center gap-2 transition-all group cursor-pointer border-1 rounded-lg  border-transparent hover:bg-[#138C9F] hover:text-white p-2 px-4 hover:border-[#138C9F]"
              >
                <span>استكشف دليل الأطباء بالكامل</span>
                <span className="transition-transform group-hover:-translate-x-1 font-sans text-lg">
                  <FontAwesomeIcon
                    icon={faArrowLeft}
                    className="transition-transform group-hover:-translate-x-1 text-sm"
                  />
                </span>
              </button>
            </div>
            ...
            <div className="lg:w-2/3 grid grid-cols-1 sm:grid-cols-3 gap-6 w-full relative z-10">
              {stepsData.map((item, index) => (
                ...
                >
                  ...
                  <h4 className="text-base font-black text-slate-900 mb-2 tracking-tight group-hover:text-[#138C9F] transition-colors duration-200">
                    {item.title}
                  </h4>
